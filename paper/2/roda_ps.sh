...
cat plota_p.py | sed "s/PREFIX/conv0.1/g" | sed "s/COLOR/${colors[$j]}/g" | sed "s/AMTSTEPS/${steps[$j]}/g" | sed "s/VALUE/1.0/g" >> /tmp/plota.py
...
cat plota_p.py | sed "s/PREFIX/conv1.0/g" | sed "s/COLOR/${colors[$j]}/g" | sed "s/AMTSTEPS/${steps[$j]}/g" | sed "s/VALUE/0.1/g" >> /tmp/plota.py
echo "${vals1[$i]}_${vals2[$i]}" >> /tmp/recargas.txt
cat /tmp/aplics${vals1[$i]}_${vals2[$i]}.txt >> /tmp/recargas.txt
j=$(expr $j + 1)
...
echo 'plt.xlim(-0.2,20.2)' >> /tmp/plota.py

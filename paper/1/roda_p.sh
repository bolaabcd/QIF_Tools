...
for j in $(seq 1 $amt); do
    ...
    cat /tmp/$arqmid | sed "s/VAL/$x/g" | sed "s/V2AL/$j/g" >> /tmp/com$j.kf
    ...
    cat /tmp/$arqmid | sed "s/VAL/$x/g" | sed "s/V2AL/$j/g" >> /tmp/sem$j.kf
    ...
    if test $(python3 -c "print(1 if ($vcom < $vsem) else 0)") -eq 1; then
        # Aplica strat
        cat $arqmid | sed "s/VAL/$x/g" | sed "s/V2AL/$j/g" >> /tmp/$arqini

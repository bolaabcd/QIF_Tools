...
cat plota_p.py | sed "s/PREFIX/conv0.1/g" | sed "s/PRREFIX/wait_once/g" | sed "s/COLOR/${colors[$j]}/g" | sed "s/AMTSTEPS/${steps[$j]}/g" | sed "s/VALUE/0.5/g" >> /tmp/plota.py
...
cat plota_p.py | sed "s/PREFIX/conv1.0/g" | sed "s/PRREFIX/wait_many/g" | sed "s/COLOR/${colors[$j]}/g" | sed "s/AMTSTEPS/${steps[$j]}/g" | sed "s/VALUE/0.5/g" >> /tmp/plota.py
echo "${vals1[$i]}_${vals2[$i]}" >> /tmp/recargas.txt
cat /tmp/aplicsconv${vals1[$i]}.${vals2[$i]}.txt >> /tmp/recargas.txt
j=$(expr $j + 1)

# UNTIL WHERE PLOT GOES (MANUAL):
echo 'plt.xlim(-0.3,30.3)' >> /tmp/plota.py
